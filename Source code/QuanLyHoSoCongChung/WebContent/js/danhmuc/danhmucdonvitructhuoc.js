$(document).ready(function() {
	loadListDanhMucDonViTrucThuoc(1);
	...
	$.ajax({
		...
		success : function(data) {
			if (data.trim() == "true") {
				...
				loadListDanhMucDonViTrucThuoc(page);
...
function loadListDanhMucDonViTrucThuoc(page) {
	$("#pageHiddenID").val(page);
	var tinh_maso = $("#TinhID").val();
	var huyen_maso = $("#SelectHuyenIDFull").val();
	$.ajax({
		type : "POST",
		url : "../ajaxdanhmuc/GetListDanhMucDonViTrucThuoc",
		async : false,
		data : ({		
			page : page,
			tinh_maso : tinh_maso,
...
function loadListDanhMucDonViTrucThuocDonViTrucThuoc(page) {
	$("#pageHiddenID").val(page);
	var tinh_maso = $("#TinhID").val();
	var huyen_maso = $("#SelectHuyenIDFull").val();
	$.ajax({
...
function changeStatusDanhMuc(danhmuc_maso) {
	...
	$.ajax({
		...
		success : function(data) {
			if (data.trim() == "true") {
				loadListDanhMucDonViTrucThuoc($("#pageHiddenID").val());
			}			
		}
	});
}
