 ...
 * {@link fckeditor.connector.exception specified exceptions} from a
...
public class Dispatcher {
	...
	UploadResponse doPost(final javax.servlet.http.HttpServletRequest request) {
		...
		// check parameters  
			...
			try {
				...
				File f = req.getFiles("NewFile")[0];
				
				//System.out.println("items: "+ f.get);
				// We upload just one file at the same time
				//FileItem uplFile = items.get(0);
				// Some browsers transfer the entire source path not just the
				// filename
				String fileName = FilenameUtils.getName(req.getFileNames("NewFile")[0]);
				...
				// Secure image check (can't be done if QuickUpload)
						...
						&& !UtilsFile.isImage(new FileInputStream(f))) {
					uploadResponse = UploadResponse.getInvalidFileTypeError();
				} else {
					...
					String newFileName = connector.fileUpload(type, context
							.getCurrentFolderStr(), sanitizedFileName, new FileInputStream(f));
