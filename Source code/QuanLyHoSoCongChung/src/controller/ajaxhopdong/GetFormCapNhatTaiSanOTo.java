...
public class GetFormCapNhatTaiSanOTo extends BaseAction {
	...
	public String executeCapNhatThongTinTaiSanOTo() {
		...
		if (hopDongTam != null) {
			...
			//Thong tin chi tiet
			TSOTo oTo = new TSOTo();
			oTo.setNhanHieu(nhanHieu);
			oTo.setLoaiXe(loaiXe);
			oTo.setMauSon(mauSon);
			oTo.setSoChoNgoi(soChoNgoi);
			oTo.setNamSanXuat(namSanXuat);
			oTo.setSoKhung(soKhung);
			oTo.setSoMay(soMay);
			oTo.setTaiTrong(taiTrong);
			oTo.setTuTrong(tuTrong);
			oTo.setGiayDangKySo(giayDangKySo);
			Date d1 = null;
			...
			oTo.setNgayCapGiayDangKy(d1);
			oTo.setNoiCapGiayDangKy(noiCapGiayDangKy);
			
			taiSan.setTaiSanOTo(oTo);
			oTo.setTaiSan(taiSan);
